  ...
  public void execute() {
    if (this.isEntityRadar) {
      if (EntityList != null && EntityList.size() > 0)
        drawEntityList(EntityList, (float)calc(this.rot), centerX + calc(this.left), centerY + calc(this.top), 
            calc(this.width), calc(this.height)); 
    } else if (EnemyList != null && EnemyList.size() > 0) {
      drawEntityList(EnemyList, (float)calc(this.rot), centerX + calc(this.left), centerY + calc(this.top), 
          calc(this.width), calc(this.height));
    } 
  }
