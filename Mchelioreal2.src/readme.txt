...
前提MOD
	Minecraft Forge 1.12.2-14.23.5.2854 以降が必要です。
	(上記バージョンで動作確認を行ってます)

Install 方法
	jar を任意のゲームディレクトリ以下のmods フォルダ内に入れて下さい。

Uninstall方法
	mods フォルダからこのMODのjarを削除して下さい。
...
更新履歴
	2023/09/18 Ver.1.1.4
		■追加機能
		・機体、コンテナ等のクリエイティブ時アイテムピックに対応
		■不具合修正
		・牽引ロープの不具合修正
		・ラぺリングが機能しない不具合修正
		・ファランクスのマシンガン連射がガンナーが操作してると単発になる不具合修正
		・固定砲台の等倍時にZ: Zoomの表示がない不具合修正
		・Mk29艦対空個艦防衛ミサイルが撃てない不具合修正
		・スカイラインGT-Rの1人称視点が低い不具合修正
		・3Dアイテム(GLTD、スティンガー等)を手に持った状態でホットバーにある製図台の表示がおかしくなる不具合修正
		・UAVステーション使用中に自分の姿が見えない不具合修正
		・製図台のGUIでおきていた不具合修正
		・レンジファインダーの充電表示の不具合修正
	2023/09/18 Ver.1.1.3
		■不具合修正
		・一部ワールドで弾を発射する等でクラッシュする不具合修正
		・マルチプレイ時にMキー、 Lキーを押すとクラッシュする不具合修正
	2023/09/11 Ver.1.1.2
		■不具合修正
		・シングルプレイ側でMOD導入時にMinecraftが起動できない不具合修正
		・発射した弾の一部でエンティティに当たるとクラッシュする不具合修正
		・爆発時にパーティクルが表示されていなかった不具合修正
	2023/09/09 Ver.1.1.1
		■追加機能
		・機体に積んでいる武器の射出位置を可視化(configのTest Mode有効時に表示される)
		■不具合修正
		・戦闘機でフレームレート制限を外すとロール回転が速くなる不具合修正
		・サーバー側でMODを導入するとクラッシュする不具合修正
		・マルチプレイ時にParticleを発生させるとクラッシュする不具合修正
		・レンチを使うとクラッシュする不具合修正
		・機体のGUIを開いてアイテムをシフトクリックするとクラッシュする不具合修正
		・スモークグレネードが真下に落下する不具合修正
		・Fim92, Fgm148, RangeFinder, GLTD, Wrenchの一人称/三人称/アイテムドロップ時の見た目・位置がおかしい不具合修正
		・カメラの不具合修正(視点, プレイヤーちらつき, ズーム機能)
	2023/09/01 Ver.1.1.0
		Minecraft 1.12.2 に対応
		ベータ版公開
		解凍方式からアドオン方式へ変更
		 ・ゲームディレクトリ以下に生成されるmcheli_addons フォルダ以下に圧縮ファイルかフォルダを入れることで機体を追加
		 ・~/mcheli_addons/{任意の名前}/assets/mcheli/{追加したい種類名}/機体設定ファイル で配置すると追加可能
		 ・階層の構成はMODのjarと同じ
