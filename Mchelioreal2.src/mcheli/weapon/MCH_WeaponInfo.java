...
public class MCH_WeaponInfo extends MCH_BaseInfo {
  ...
  public boolean validate() throws Exception {
    ...
    if (this.destruct)
      this.delay = 1000000; 
    this.angle = (float)(Math.atan2(this.radius, this.length) * 180.0D / Math.PI);
    return true;
  }
    ...
    if (item.compareTo("displayname") == 0) {
      ...
      if (item.compareTo("soundvolume") == 0) {
        ...
        if (s.length >= 7) {
          ...
          this.listMuzzleFlash.add(new MuzzleFlash(toFloat(s[0]), toFloat(s[1]), 0.0F, 
                toInt(s[2]), toFloat(s[3]) / 255.0F, toFloat(s[4]) / 255.0F, toFloat(s[5]) / 255.0F, 
                toFloat(s[6]) / 255.0F, 1));
        } 
        ...
        if (s.length >= 9) {
          ...
          this.listMuzzleFlashSmoke.add(new MuzzleFlash(toFloat(s[0]), toFloat(s[2]), 
                toFloat(s[3]), toInt(s[4]), toFloat(s[5]) / 255.0F, toFloat(s[6]) / 255.0F, 
                toFloat(s[7]) / 255.0F, toFloat(s[8]) / 255.0F, toInt(s[1], 1, 1000)));
        } 
      ...
      } else if (item.equalsIgnoreCase("BulletColorInWater") || item.equalsIgnoreCase("BulletColor") || item
        .equalsIgnoreCase("SmokeColor")) {
        String[] s = data.split("\\s*,\\s*");
        if (s.length >= 4) {
        ...
        if (s.length >= 2) {
          ...
          if (className.equals("player")) {
            Class<EntityPlayer> clazz1 = EntityPlayer.class;
          } else if (className.equals("heli") || className.equals("helicopter")) {
            Class<MCH_EntityHeli> clazz1 = MCH_EntityHeli.class;
          } else if (className.equals("plane")) {
            Class<MCP_EntityPlane> clazz1 = MCP_EntityPlane.class;
