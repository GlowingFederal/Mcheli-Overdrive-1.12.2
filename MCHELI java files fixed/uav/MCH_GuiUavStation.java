...
public class MCH_GuiUavStation extends W_GuiContainer {
  ...
  protected void drawGuiContainerForegroundLayer(int param1, int param2) {
    ...
    if (!item.isEmpty() && item.getItem() instanceof mcheli.plane.MCP_ItemPlane)
      MCP_PlaneInfo mCP_PlaneInfo = MCP_PlaneInfoManager.getFromItem(item.getItem()); 
    if (!item.isEmpty() && item.getItem() instanceof mcheli.helicopter.MCH_ItemHeli)
      MCH_HeliInfo mCH_HeliInfo = MCH_HeliInfoManager.getFromItem(item.getItem()); 
    if (!item.isEmpty() && item.getItem() instanceof mcheli.tank.MCH_ItemTank)
      mCH_TankInfo = MCH_TankInfoManager.getFromItem(item.getItem()); 
    if (item.isEmpty() || (mCH_TankInfo != null && ((MCH_AircraftInfo)mCH_TankInfo).isUAV)) {
      if (this.uavStation.getKind() <= 1) {
        drawString("UAV Station", 8, 6, 16777215);
      } else if (item.isEmpty() || ((MCH_AircraftInfo)mCH_TankInfo).isSmallUAV) {
