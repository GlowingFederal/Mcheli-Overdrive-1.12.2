    ...
    try {
      ...
      if (this.all) {
        ...
        this
          ...
          .ammo = new short[] { data.readShort() };
        this
          
          .restAmmo = new short[] { data.readShort() };
      } 
    } catch (Exception e) {
      e.printStackTrace();
    } 
    ...
    if (target == null) {
      for (int i = 0; i < ac.getSeatNum() + 1; i++) {
        ...
        if (e instanceof EntityPlayer)
          W_Network.sendToPlayer((W_PacketBase)s, (EntityPlayer)e); 
      } 
    } else {
      W_Network.sendToPlayer((W_PacketBase)s, target);
    } 
  }
}
