...
void main(){
    vec4 center = texture2D(DiffuseSampler, tex);
    vec4 up     = texture2D(DiffuseSampler, tex + vec2(        0.0, -oneTexel.y));
    vec4 up2    = texture2D(DiffuseSampler, tex + vec2(        0.0, -oneTexel.y) * 2.0);
    vec4 down   = texture2D(DiffuseSampler, tex + vec2( oneTexel.x,         0.0));
    vec4 down2  = texture2D(DiffuseSampler, tex + vec2( oneTexel.x,         0.0) * 2.0);
    vec4 left   = texture2D(DiffuseSampler, tex + vec2(-oneTexel.x,         0.0));
    vec4 left2  = texture2D(DiffuseSampler, tex + vec2(-oneTexel.x,         0.0) * 2.0);
    vec4 right  = texture2D(DiffuseSampler, tex + vec2(        0.0,  oneTexel.y));
    vec4 right2 = texture2D(DiffuseSampler, tex + vec2(        0.0,  oneTexel.y) * 2.0);
